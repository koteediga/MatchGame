...
import WinCart from './Component/WinCart'
...
class App extends Component {
  ...
  //   componentDidMount() {
  //     this.timerId = this.setInterval(this.time, 1000)
  //   }
  ...
  thumbnailImagegeneration = id => {
    const {k, imageId} = this.state
    const randomndex = Math.floor(Math.random() * imagesList.length)
    const imageurl = imagesList[randomndex].imageUrl
    const id1 = imagesList[randomndex].id
    console.log(id)
    console.log(imageId)
    if (k.includes(id) || id !== imageId) {
      this.setState({
        isActive: true,
      })
      clearInterval(this.timerId)
  ...
  render() {
    const {activeId, randomImage, count, score, isActive} = this.state
    const filteritems = imagesList.filter(each => each.category === activeId)
    const container = isActive ? (
      <WinCart score={score} reStart={this.reStart} />
    ) : (
      tabsList.map(each => (
        <Second key={each.tabId} ChangeTab={this.ChangeTab} resourse={each} />
      ))
    )

    const classname = isActive ? 'imagecart' : ''
    return (
      <div>
        <ul>
        ...
        <ul className={classname}>
          {filteritems.map(each => (
            <ImageCart
              item={each}
              key={each.id}
              thumbnailImagegeneration={this.thumbnailImagegeneration}
            />
          ))}
        </ul>
      </div>
